...
class Chess(commands.Cog):
    ...
    async def get_player(self,ctx,*,username):
        try:
            data = await chess.get_player_profile(username).json
            data1=await chess.is_player_online(username).json
            data2=await chess.get_player_clubs(username).json
            ...
            for i in data2['clubs']:
                await profile.add_field(name="Club:",value=i['name'],inline=False)
    ...
    async def print_leaderboards(self,ctx):
            ...
            await data = chess.get_leaderboards().json
    ...
    async def get_player_rating(self,ctx,*,username):
            ...
            await data = chess.get_player_stats(username).json
            await categories = ['chess_blitz', 'chess_rapid', 'chess_bullet']
            for category in categories:
                    await ctx.send('Category:'+str(category))
